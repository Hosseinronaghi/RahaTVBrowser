...
final class BrowserStore {
    ...
    BrowserStore(Context context) {
        prefs = context.getSharedPreferences("browser_store", Context.MODE_PRIVATE);
    }

    List<Entry> favorites() { return read("favorites", 40); }
    List<Entry> history() { return read("history", 10); }

    boolean isFavorite(String url) {
        for (Entry e : favorites()) if (e.url().equals(url)) return true;
        return false;
    }
    ...
    void toggleFavorite(String title, String url) {
        ...
        if (!removed && url != null && !url.isBlank()) list.add(0, new Entry(cleanTitle(title, url), url));
        write("favorites", list, 40);
    }

    void addHistory(String title, String url) {
        if (url == null || url.startsWith("file:///android_asset") || url.equals("about:blank")) return;
    ...
    private void write(String key, List<Entry> list, int max) {
        JSONArray a = new JSONArray();
        try {
            for (int i = 0; i < Math.min(list.size(), max); i++) {
                JSONObject o = new JSONObject();
                o.put("title", list.get(i).title());
                o.put("url", list.get(i).url());
                a.put(o);
            }
        } catch (Exception ignored) {}
